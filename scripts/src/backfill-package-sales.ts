 ...
 *   1. Find financial_entries where:
 *        - tenantId matches
 *        - type = 'receita'
 *        - date is within ±1 day of the earliest appointment in the group
 *        - description contains the package name (case-insensitive)
 *   2. If exactly one entry matches → use its amount as totalPrice
 *   3. If zero or multiple entries match → totalPrice = NULL (ambiguous)
...
async function main() {
  ...
  // 1. Get all distinct (tenantId, clientId, petId, packageId, recurringGroupId) groups
  ...
  for (const group of toBackfill) {
    ...
    if (!recurringGroupId || !packageId || !tenantId || !clientId) {
      skipped++;
      continue;
    }
    ...
    const packageName = pkgRow?.name ?? null;
    ...
    // Price inference: look in financial_entries within ±1 day of saleDate
    // matching tenantId, type=receita, and package name in description
    ...
    if (packageName) {
      ...
      const candidates = await db
        .select({ amount: financialEntriesTable.amount, description: financialEntriesTable.description, date: financialEntriesTable.date })
        .from(financialEntriesTable)
        .where(
          and(
            eq(financialEntriesTable.tenantId, tenantId),
            eq(financialEntriesTable.type, "receita"),
            gte(financialEntriesTable.date, dayBeforeStr),
            lte(financialEntriesTable.date, dayAfterStr),
      ...
      if (candidates.length === 1) {
        ...
        ambiguityReason = `No matching financial_entries found within ±1 day of ${saleDate} for package "${packageName}"`;
      } else {
        ambiguityReason = `${candidates.length} matching financial_entries found (amounts: ${candidates.map(c => c.amount).join(", ")}), ambiguous`;
      }
    } else {
      ambiguityReason = "Package name unknown (package may have been deleted)";
    }
    ...
    // Insert with totalPrice=null when ambiguous (still creates the row for tracking)
    try {
      ...
      const priceDisplay = totalPrice != null ? `R$ ${parseFloat(totalPrice).toFixed(2)}` : "NULL (pendente revisão)";
      console.log(`  ✔ ${recurringGroupId.substring(0, 8)}… | ${packageName ?? "?"} | ${petName ?? "?"} | ${weeks} sessões | preço: ${priceDisplay}`);
      created++;
    } catch (err: unknown) {
  ...
  if (pendingReview.length > 0) {
    ...
    for (const g of pendingReview) {
      ...
      console.log(`  Tenant  : ${g.tenantId} | Pacote: ${g.packageName ?? "?"} | Pet: ${g.petName ?? "?"}`);
      console.log(`  Venda   : ${g.saleDate} | ${g.weeks} sessões`);
      console.log(`  Motivo  : ${g.reason}`);
      console.log(`  Ação    : UPDATE package_sales SET total_price = <valor> WHERE recurring_group_id = '${g.recurringGroupId}';`);
      console.log("─".repeat(80));
    }
  }
